    ...
    return function getUserInfo(id, callback) {
        ...
        defaultFuncs
            ...
            .then(function (resData) {
                if (resData.error) {
                    const errMessage = resData.errorDescription || "Unknown error occurred.";
                    const errorDetails = {
                        errorCode: resData.error,
                        errorSummary: resData.errorSummary || "Error summary not available",
                        errorDescription: errMessage,
                        errorPayload: resData.payload
                    };
                    throw new Error(JSON.stringify(errorDetails));
                }
                if (!resData.payload || !resData.payload.profiles) {
                    throw new Error("Invalid response format: Missing profiles data.");
                }
            ...
            .catch(function (err) {
                ...
                const friendlyError = {
                    message: "Failed to fetch user information. Please try again later.",
